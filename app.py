...
# VISNING AV KURSER: 3 kolumner med checkboxar

st.subheader("🔍 Välj kurser")

cols = st.columns(3)
selected_courses = []
# Visa högst 9 kurser (ändra .head(n) vid behov)
...
# Möjlighet att maila valda kurser som en HTML-tabell
...
if st.button("Skicka information via mail"):
    if selected_courses and mail.strip():
        ...
        # Skapa en mailto-länk med HTML i body
        ...
        st.markdown(
            ...
            f"_OBS! Inte alla e-postklienter visar HTML korrekt via mailto-länkar._",
            unsafe_allow_html=True
        )
    else:
